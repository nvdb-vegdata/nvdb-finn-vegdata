...
- The derived stedfesting filter is used to fetch vegobjekter (same as manual stedfesting mode)
...
- When clipping is enabled, vegobjekter are also filtered client-side against the clipped veglenke ranges so objects on overlapping veglenker but outside the polygon are removed, unless the veglenkesekvens result hit the configured limit
...
- The popup has a close button (×) in the upper right corner that dismisses it without deactivating the mode
...
### Vegnett API (Segmentert)
| Endpoint | Purpose |
|----------|---------|
| `GET /api/v4/veglenkesekvenser/segmentert` | Query segmentert vegnett by vegsystemreferanse (used in Vegsystemreferanse mode) |

Query parameters for segmentert:
- `vegsystemreferanse`: Vegsystemreferanse string (e.g., "FV6666 S1")
- `antall`: Limit results (default 1000 in this app)
...
When querying vegobjekter in polygon mode, the app sends the drawn polygon directly to the Uberiket stream endpoint and parses the NDJSON response. Polygon clipping still does not change the API request, but when clipping is enabled the app filters streamed vegobjekter client-side against the clipped veglenke ranges when the veglenkesekvens result is not truncated.
...
When the user clicks a download option, the app generates the CSV directly from the streamed result set that is already loaded in memory.
